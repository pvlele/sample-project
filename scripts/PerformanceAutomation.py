...
def housekeeping():
    ...
    if(path.exists(path.join(project_path,"results", "build-status-report-data.txt"))):
        rm_status_file = f'rm -r {path.join(project_path,"results", "build-status-report-data.txt")}'
        subprocess.Popen(rm_status_file, shell=True).wait()
    ...
    with open(f"{ms_ips_path}", "r") as IP_list:
        for ip in IP_list:
            ...
            while not file_exist:
                stat_cmd = f'ssh {ms_user}@{ip} stat {hardware_stat_path}/CPUUSAGE*'
                try:
                    stat_result = subprocess.run(["scp",f'{ms_user}@{ip}:{hardware_stat_path}/CPUUSAGE*',f'{path.join(test_report_path, "hardwareUtilization/")}'])
                    if stat_result.returncode == 0:
                        subprocess.run(["scp",f'{ms_user}@{ip}:{hardware_stat_path}/CPUUSAGE*',f'{path.join(test_report_path, "hardwareUtilization/")}'])
                    ...
                    logging.info(f'File {hardware_stat_path}/CPUUSAGE_{ip}.txt not found on {ip}')
                    time.sleep(10)
...
if is_master:
    ...
    execute_postScript()
    find_perf_anomaly()
    status_file_bkp = f'{project_path}/results/build-status-report-data_bkp.txt'
    bkp_cmd = f'cp {status_file} {status_file_bkp}'
    subprocess.Popen(bkp_cmd, shell=True)
